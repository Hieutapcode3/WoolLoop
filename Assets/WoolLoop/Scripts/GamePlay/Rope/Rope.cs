...
public class Rope : MonoBehaviour, IOnRecycle
{
    ...
    [Tooltip("Applies a local visual offset from start/end attachment points.")]
    public Vector3 endpointLocalOffset = Vector3.zero;
    [Tooltip("Additional world-space offset (for example Y = -0.05 to move both ends down globally).")]
    public Vector3 endpointWorldOffset = Vector3.zero;
    ...
    private void Update()
    {
        ...
        if (AreEndPointsValid())
        {
            ...
            prevStartPointPosition = GetStartPointPosition();
            prevEndPointPosition = GetEndPointPosition();
    ...
    private void SetSplinePoint()
    {
        ...
        Vector3 startPos = GetStartPointPosition();
        Vector3 endPos = GetEndPointPosition();

        if (midPoint != null)
        {
            midPoint.position = GetRationalBezierPoint(startPos, mid, endPos, midPointPosition, StartPointWeight, midPointWeight, EndPointWeight);
        }

        lineRenderer.SetPosition(0, startPos);
        for (int i = 1; i < linePoints; i++)
        {
            Vector3 p = GetRationalBezierPoint(startPos, mid, endPos, i / (float)linePoints, StartPointWeight, midPointWeight, EndPointWeight);
            lineRenderer.SetPosition(i, p);
        }

        lineRenderer.SetPosition(linePoints, endPos);
    }
    ...
    {
        Vector3 startPointPosition = GetStartPointPosition();
        Vector3 endPointPosition = GetEndPointPosition();
    ...
    public Vector3 GetPointAt(float t)
    {
        ...
        return GetRationalBezierPoint(GetStartPointPosition(), currentValue, GetEndPointPosition(), t, StartPointWeight, midPointWeight, EndPointWeight);
    }
    ...
    public void SetStartPoint(Transform newStartPoint, bool instantAssign = false)
    {
        ...
        prevStartPointPosition = startPoint == null ? Vector3.zero : GetStartPointPosition();
    ...
    public void SetEndPoint(Transform newEndPoint, bool instantAssign = false)
    {
        ...
        prevEndPointPosition = endPoint == null ? Vector3.zero : GetEndPointPosition();
    ...
    {
        var startPointMoved = GetStartPointPosition() != prevStartPointPosition;
        var endPointMoved = GetEndPointPosition() != prevEndPointPosition;
    ...
    private Vector3 GetStartPointPosition()
    {
        ...
        return startPoint.TransformPoint(endpointLocalOffset) + endpointWorldOffset;
    }
    ...
    private Vector3 GetEndPointPosition()
    {
        ...
        return endPoint.TransformPoint(endpointLocalOffset) + endpointWorldOffset;
    }
